...
export default class Admin2 extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      listProduct2: [], // variabel array yang digunakan untuk menyimpan data Firebase
    };
  }
  ...
  handleHapusProduct = (idProduct) => {
    ...
    const { listProduct2 } = this.state;
    const newState = listProduct2.filter((data) => {
      return data.id !== idProduct;
    });
    this.setState({ listProduct2: newState });
  };
  ...
  handleTombolSimpan = (event) => {
    ...
    if (id && nama && gambar && harga) {
      ...
      const { listProduct2 } = this.state;
      const indeksProduct = listProduct2.findIndex((data) => {
        return data.id === id;
      });
      listProduct2[indeksProduct].nama = nama;
      listProduct2[indeksProduct].gambar = gambar;
      listProduct2[indeksProduct].harga = harga;
      this.setState({ listProduct2 });
    } else if (nama && gambar && harga) {
      // jika data belum di server
      const id = new Date().getTime().toString();
      const { listProduct2 } = this.state;
      listProduct2.push({ id, nama, gambar, harga });
      this.setState({ listProduct2 });
  ...
  render() {
    return (
      <div className="post-product">
        ...
        <h2 className="text-center my-5">Data Produk Front Loading</h2>
        <div className="table-responsive">
          <table className="table table-striped">
            ...
            <tbody>
              {this.state.listProduct2.map((product2) => {
                return (
                  <Post
                  key={product2.id}
                  nama={product2.nama}
                  gambar={product2.gambar}
                  harga={product2.harga}
                  hapusProduct={() => this.handleHapusProduct(product2.id)}
                />
                );
              })}
            </tbody>
